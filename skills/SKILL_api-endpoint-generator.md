...
description: >
  Generates complete API endpoint implementations following the project's repository → 
  service → route pattern. Use when: user says "create endpoint", "add API for", "build 
  ...
  before generating — consistency with the existing codebase is critical.
---
...
1. **Read existing routes** — Match patterns, naming conventions, error handling style
2. **Check shared types** — Reuse existing interfaces where possible
3. **Verify database tables** — Confirm the table exists or note that a migration is needed
4. **Check API.md** — Ensure this endpoint isn't already documented (avoid duplication)
5. **Review REQUIREMENTS.md** — Understand the acceptance criteria this endpoint serves
...
```
1. packages/shared/src/types/index.ts      → TypeScript interfaces
2. packages/shared/src/validation/index.ts  → Zod validation schemas
3. packages/api/src/repositories/[domain]Repo.ts → Data access (Knex.js)
...
- Format database rows into API response shapes (snake_case → camelCase)
...
// GET /api/items/:id — Get item detail (public)
