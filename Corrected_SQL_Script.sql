-- Corrected SQL Script for FoodGenie Database Seeding
-- Handles Foreign Key constraints by dropping tables in correct dependency order

USE FoodGenie;
GO

-- 1. DROP TABLES (Child tables first, then parents)
...
CREATE TABLE Users (
    ...
    stats_json NVARCHAR(MAX), -- Storing JSON objects as string for simplicity in SQL Express
    preferences_json NVARCHAR(MAX)
);
GO

-- FoodNodes Table (Materialized Path Pattern)
CREATE TABLE FoodNodes (
    id INT PRIMARY KEY,
    name NVARCHAR(100) NOT NULL,
    path NVARCHAR(255) NOT NULL,
    parent_id INT,
    type NVARCHAR(50) NOT NULL, -- 'category', 'generic_food', 'branded_product'
    description NVARCHAR(MAX),
    image NVARCHAR(255),
    nutrition_json NVARCHAR(MAX),
    pairings_json NVARCHAR(MAX),
    storage NVARCHAR(MAX),
    FOREIGN KEY (parent_id) REFERENCES FoodNodes(id)
);
GO
...
CREATE TABLE RecipeIngredients (
    ...
    FOREIGN KEY (food_node_id) REFERENCES FoodNodes(id)
);
GO
...
CREATE TABLE PantryItems (
    ...
    quantity FLOAT,
    unit NVARCHAR(50),
    location NVARCHAR(50), -- 'fridge', 'pantry', 'freezer', 'spice_rack'
...
CREATE TABLE Friends (
    ...
    status NVARCHAR(50), -- 'connected', 'pending_incoming', 'pending_outgoing'
    mutual_friends INT,
...
CREATE TABLE Activities (
    id INT PRIMARY KEY,
    user_id INT NOT NULL,
    type NVARCHAR(50), -- 'recipe_posted', 'review_added', 'joined_challenge'
    title NVARCHAR(200),
    description NVARCHAR(MAX),
...
INSERT INTO FoodNodes (id, name, path, parent_id, type, description, image, nutrition_json, pairings_json, storage) VALUES
(6, 'Pork', '/1/2/5/6/', 5, 'category', NULL, NULL, NULL, NULL),
(7, 'Poultry', '/1/2/5/7/', 5, 'category', NULL, NULL, NULL, NULL);
...
INSERT INTO FoodNodes (id, name, path, parent_id, type, description, image, nutrition_json, pairings_json, storage) VALUES
(90, 'Oscar Mayer Bacon', '/1/2/5/6/8/9/90/', 9, 'branded_product', 'Specific brand of bacon.', NULL, NULL, NULL, NULL);
...
(1, 4, 3, 'large', NULL), -- Eggs
(1, 14, 1, 'cup', NULL),   -- Cheddar
(1, 27, 400, 'g', NULL),   -- Spaghetti
(1, 32, 2, 'cloves', NULL),-- Garlic
(1, 9, 200, 'g', NULL);    -- Bacon

-- Chicken Salad
INSERT INTO RecipeIngredients (recipe_id, food_node_id, quantity, unit, notes) VALUES
(2, 11, 2, 'pieces', NULL), -- Chicken Breast
(2, 31, 1, 'medium', NULL), -- Onion
(2, 34, 1, 'head', NULL);   -- Lettuce

-- Pancakes
INSERT INTO RecipeIngredients (recipe_id, food_node_id, quantity, unit, notes) VALUES
(3, 4, 2, 'large', NULL),   -- Eggs
(3, 12, 1, 'cup', NULL),    -- Milk
(3, 25, 2, 'cups', NULL),   -- Flour
(3, 15, 50, 'g', NULL);     -- Butter
...
-- Activities
INSERT INTO Activities (id, user_id, type, title, description, timestamp, likes, comments, image) VALUES
(1, 2, 'recipe_posted', 'Posted a new recipe', 'Just perfected my grandmothers spicy ramen recipe! Check it out.', '2 hours ago', 24, 5, 'https://images.unsplash.com/photo-1569718212165-3a8